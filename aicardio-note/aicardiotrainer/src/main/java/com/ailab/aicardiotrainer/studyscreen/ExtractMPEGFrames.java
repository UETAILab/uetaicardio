...
public class ExtractMPEGFrames {
    ...
    private static final File FILES_DIR = new File("/storage/emulated/0/Download") ; // Environment.getExternalStorageDirectory();
    private static final String INPUT_FILE = "video.mp4";
    private static final int MAX_FRAMES = 100000;       // stop extracting after this many
    ...
    public ArrayList<Bitmap> extractMPEGFrames(String fileNameMp4) throws IOException {
...
//            File inputFile = new File(FILES_DIR, INPUT_FILE);   // must be an absolute path
            File inputFile = new File(fileNameMp4);
            ...
            int saveWidth = format.getInteger(MediaFormat.KEY_WIDTH);
            int saveHeight = format.getInteger(MediaFormat.KEY_HEIGHT);

//            int maxBFrame = format.getInteger(MediaFormat.KEY_MAX_B_FRAMES);
//
//            Log.w(TAG, "maxBFrame: " + maxBFrame);

//            int numFrame = format.getInteger(MediaFormat.Fr);
            ...
            bitmaps = doExtract(extractor, trackIndex, decoder, outputSurface);
     ...
     * Work loop.
     */
    static ArrayList<Bitmap> doExtract(MediaExtractor extractor, int trackIndex, MediaCodec decoder,
...
//                        if (VERBOSE) Log.d(TAG, "awaiting decode of frame " + decodeCount);
                        ...
                        decodeCount++;
//                        Log.w(TAG, "awaiting decode of frame " + decodeCount);
                        Bitmap bitmap = outputSurface.saveFrame();
                        if (bitmap != null) {
                            bitmaps.add(bitmap);
                        }
//                        bitmaps.add(outputSurface.saveFrame());
...
//                Log.w(TAG, "Saved " + mWidth + "x" + mHeight + " frame as '" + filename + "'");
            }
//            if (true) {
////                Log.w(TAG, "Saved " + mWidth + "x" + mHeight + " frame as '" + filename + "'");
//            }
