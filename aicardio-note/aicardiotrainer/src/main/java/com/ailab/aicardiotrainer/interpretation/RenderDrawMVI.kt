...
import com.ailab.aicardiotrainer.getAreaPath
import com.ailab.aicardiotrainer.getLengthPoint
...
//                        else -> 0xFF808000.toInt()
        ...
        fun drawBoundary(view: InterpretationCanvasView, canvas: Canvas?, boundary: JSONArray, paint: Paint) {
            try {
...
//                }
        ...
        fun drawLine(view: InterpretationCanvasView, canvas: Canvas?, p1: JSONObject?, p2: JSONObject?, paint: Paint) {
            try {
                if (p1 == null || p2 == null)
                    return
        ...
        fun drawPoint(view: InterpretationCanvasView, canvas: Canvas?, p: JSONObject, paint: Paint) {
            val point = view.getScreenCoordinate(p)
            canvas?.drawCircle(point[0], point[1], 2.0F, paint)
        }

        fun drawText(view: InterpretationCanvasView, canvas: Canvas?, text: String, p: JSONObject, paint: Paint) {
            val point = view.getScreenCoordinate(p)
            canvas?.drawText(text, point[0], point[1], paint)
        }


        fun drawPointKnot(view: InterpretationCanvasView, canvas: Canvas?, knots: JSONArray, paint: Paint){
            try {
    ...
    fun process(interpretationActVM: InterpretationActVM, interpretationViewEvent: InterpretationViewEvent) {
        ...
        when (interpretationViewEvent) {
            is InterpretationViewEvent.RenderDraw -> {
                interpretationActVM.viewStates().value?.let {
                    interpretationActVM.reduce(RenderDrawReducer(interpretationActVM, it, interpretationViewEvent ))
...
//            }
    ...
    inner class RenderDrawReducer(viewModel: InterpretationActVM, viewState: InterpretationViewState, val viewEvent: InterpretationViewEvent.RenderDraw)
    ...
    fun drawByType(viewState: InterpretationViewState, viewModel: InterpretationActVM, viewEvent: InterpretationViewEvent.RenderDraw, typeDraw: String) {
...
//        val tool = viewModel.getCurrentTool()
        ...
        val currentToolName = viewModel.getToolNameUsing()
...
//
                ...
                if (currentToolName == InterpretationActVM.TOOL_DRAW_BOUNDARY && toolClickedType == true && isGls == false)
                    drawPointKnot(
                        view,
                        canvas,
...
//
                ...
                if (currentToolName == InterpretationActVM.TOOL_DRAW_BOUNDARY && toolClickedType == true && isGls == true)
                    drawPointKnot(
                        view,
                        canvas,
...
//                Log.w(TAG, "pointLengthTool: ${pointLengthTool}")
                drawPointLengthTool(
                    viewState,
                    view,
                    canvas,
    ...
    private fun drawBoundaryAreaTool(viewState: InterpretationViewState, view: InterpretationCanvasView, canvas: Canvas?, boundary: JSONArray, paintText: Paint, paintLine: Paint, paintPoint: Paint) {
        canvas?.let {
            try {
                repeat(boundary.length()) {
                    val paths = boundary.getJSONArray(it)
    ...
    private fun drawPointLengthTool(viewState: InterpretationViewState, view: InterpretationCanvasView, canvas: Canvas?, points: JSONArray, paintText: Paint, paintLine: Paint, paintPoint: Paint) {
        canvas?.let {
            try {
