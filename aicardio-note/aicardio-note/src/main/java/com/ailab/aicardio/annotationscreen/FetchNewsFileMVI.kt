...
import com.ailab.aicardio.annotationscreen.interfaces.LoadingProgressListener
import com.ailab.aicardio.converJSONObjectOldVerion
...
class FetchNewsFileMVI {

    companion object {
        // For Singleton instantiation
        @Volatile
        private var instance: FetchNewsFileMVI? = null
        ...
        const val TAG = "FetchNewsFileUniDirectionMVI"
...
//                annotationActivity.iv_draw_canvas.invalidate()
                ...
                val currentPosition = viewState.getCurrentPosition()


                annotationActivity.newsRvFolderAdapter.setCurrentPosition(currentPosition)
    ...
    fun process(annotationActVM: AnnotationActVM, annotationViewEvent: AnnotationViewEvent) {
        when (annotationViewEvent) {
            ...
            is AnnotationViewEvent.NewProgess -> {
                annotationActVM.reduceStateEffectObject(null, AnnotationViewEffect.LoadingProgress(annotationViewEvent.progress))
            }
            ...
            is AnnotationViewEvent.FileFetchedSuccess -> {
                ...
                if (annotationViewEvent.viewState.bitmaps.size > 0) {
//                    Log.w(TAG, "annotationActVM size: ${annotationActVM.numFrame}")
    ...
    inner class FetchNewsFileReducerAsync(viewModel: AnnotationActVM, viewState: AnnotationViewState, val viewEvent: AnnotationViewEvent.FetchNewsFile)
        ...
        override fun reduce(): AnnotationStateEffectObject {
            Log.w(FetchNewsFolderMVI.TAG, "FetchNewsFileReducerAsync ${viewEvent.file}")
            ...
            viewModel.viewModelScope.launch {
                ...
                when (val readResult = dicomRepository.getDatasetAndBitmaps(file, listener)) {
                    ...
                    is LCE.Result -> if (readResult.error) {
                        Log.w(AnnotationActVM.TAG, "after call to getDatasetAndBitmaps fail")

                        viewModel.viewStates().value?.let {
                            FetchNewsFileMVI.process(viewModel, AnnotationViewEvent.FileFetchedError(readResult))
                        }
        ...
        override suspend fun onProgress(progress: Long) = withContext(Dispatchers.Main) {
            FetchNewsFileMVI.process(viewModel, AnnotationViewEvent.NewProgess(progress))
        }
                ...
                when (val readResult = annotationRepository.getAnnotationFromFile(fileJSON)) {
                    ...
                    is LCE.Result -> {
                        if (readResult.error) {
                            ...
                            FetchNewsFileMVI.process(viewModel, AnnotationViewEvent.FileFetchedSuccess(viewStateIfError,
...
//                        Log.w(TAG, "start launch read file ${readResult.error} ${readResult.message} ${readResult.data}$")
                            ...
                            val manual_annotation = if(
                                readResult.data.has(AnnotationActVM.MANUAL_ANNOTATION) &&
                                readResult.data.getJSONArray(AnnotationActVM.MANUAL_ANNOTATION).length() == bitmaps.size) {
                                DicomAnnotation(readResult.data.getJSONArray(
                            ...
                            val machine_annotation = if(readResult.data.has(AnnotationActVM.MACHINE_ANNOTATION)
                                && readResult.data.getJSONArray(
                            ...
                            if (readResult.data.has("version")) { // VERSION
                                ...
                                FetchNewsFileMVI.process(viewModel, AnnotationViewEvent.FileFetchedSuccess(viewEffect = effect, viewState = state))
            ...
            return AnnotationStateEffectObject(null, AnnotationViewEffect.ShowToast("Loading annotation"))
        }
