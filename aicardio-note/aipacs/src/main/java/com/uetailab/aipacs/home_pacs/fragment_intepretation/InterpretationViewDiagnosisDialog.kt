...
import kotlinx.android.synthetic.main.interpretation_view_dialog_diagnosis.*
import org.json.JSONArray
import org.json.JSONObject

class InterpretationViewDiagnosisDialog(
    val activity: Activity,
    val listener: OnDiagnosisEnteredListener,
...
//        setContentView(R.layout.dialog_diagnosis)
        ...
        val layout: View = inflater.inflate(R.layout.interpretation_view_dialog_diagnosis, null)
        layout.minimumWidth = (displayRectangle.width() * 0.9f).toInt()
        layout.minimumHeight = (displayRectangle.height() * 0.9f).toInt()

        setContentView(layout)
        Log.w(TAG, "interpretationViewVM Diagnosis: ${interpretationViewVM.dicomDiagnosis}")
        bt_cancel_dialog_diagnosis.setOnClickListener {
            cancel()
        }

        bt_ok_dialog_diagnosis.setOnClickListener {
            onOKClicked()
        }
        ...
        radioGrid.setOnCheckedChangeListener(object : InterpretationViewRadioGridGroup.OnCheckedChangeListener {
            override fun onCheckedChanged(group: InterpretationViewRadioGridGroup?, checkedId: Int) {
    ...
    private fun prepareImageViews(view: InterpretationViewStudyPreviewCanvasView) {
...
//                    4 -> "PST_S"
                    ...
                    val indexDrawSector = getIndexOfHeartSector(interpretationViewVM.boundaryHeart, Point(x, y), indexSector )
                    if (indexDrawSector != -1) {
                        val heartBoundary = interpretationViewVM.boundaryHeart.getJSONObject(indexDrawSector).getJSONArray("boundary")
                        interpretationViewVM.viewStates().value?.let {
                            InterpretationViewDrawTouchEventMVI.drawMultiPolygon(it, view, canvas, heartBoundary, paint, drawTextArea=false)
                        }
