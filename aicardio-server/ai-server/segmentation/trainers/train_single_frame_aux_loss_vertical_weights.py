...
from segmentation.trainers.datasets import HDSingleFrameDataset, single_image_normalize, torch_hub_normalize
...
class TrainerAuxLossVerticalWeights(Trainer):
    ...
    def get_loss(self, ypred, masks, batch):
        ...
        if type(ypred) != tuple:
            ...
            hd_loss = hdloss(y_pred, masks, batch['hd'].to(y_pred.device))
            vloss = verticalloss(y_pred, masks, batch['vertical_weights'].to(y_pred.device))
            ...
            vweights = batch['vertical_weights'].to(y_pred.device)
            vloss = verticalloss(y_pred, masks, vweights)\
                    + verticalloss(aux_y_pred_2, masks, vweights)\
                    + verticalloss(aux_y_pred_3, masks, vweights)
            
        loss = wce_loss + soft_iou_loss + hd_loss + vloss
        return loss
...
#     additional_process = single_image_normalize
    ...
    # carry out training
    trainer = TrainerAuxLossVerticalWeights(datasets, train_config)
    trainer.train()
