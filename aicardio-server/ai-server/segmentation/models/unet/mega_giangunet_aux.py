...
class EncoderBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dilation=1, n_layers=1):
        super().__init__()
        layers = []
        layers.append(SEBasicBlock(in_channels, out_channels, dilation=dilation))
        for _ in range(1, n_layers):
            layers.append(SEBasicBlock(out_channels, out_channels, dilation=dilation))
