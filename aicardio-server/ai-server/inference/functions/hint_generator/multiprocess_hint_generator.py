...
def get_input_queue(chamber_data, chamber):
    ...
    for i, dicom_path in enumerate(chamber_data):
        if chamber_data[dicom_path]["chamber"] == chamber:
            input_queue.put(dicom_path)
            n_loaded_files += 1
    print(f"Loaded {n_loaded_files} DICOM files to input queue")
    return input_queue
...
def save_result(item, out_dir, gls_extend_size):
    result = {
        "path_file": item["dicom_path"],
        "siuid": item["metadata"]["siuid"],
        "sopiuid": item["metadata"]["sopiuid"],
        "dicomDiagnosis": {
...
def get_out_path(item, out_dir):
    ...
    siuid = item["metadata"]["siuid"]
    sopiuid = item["metadata"]["sopiuid"]
    out_path = os.path.join(out_dir, f"{siuid}__{sopiuid}__{dicom_id}.json")
    return out_path


if __name__ == "__main__":
    r"""
    json file should be of the form
        {
            <dicom_path 1>: ...,
            <dicom_path 2>: ...,
            ...
    ...
    parser.add_argument("--json_path", type=str, help="Path to JSON chamber file")
    ...
    parser.add_argument("--gls_extend_size", type=float, default=0.05)
    parser.add_argument("--n_threads", type=int, default=2)
    args = parser.parse_args()

    pipeline_config = load_pipeline_config(args.pipeline_config)
    run(args.json_path, args.chamber, pipeline_config, args.out_dir,
