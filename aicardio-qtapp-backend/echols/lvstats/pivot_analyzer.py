...
class PivotAnalyzer:
    ...
    def detect_pivots(self, contour, peakpoint, basepoints, n_segments_per_side=3):
        ...
        Args:
            ...
            basepoints (dict): LV base points. This is a Python dict with keys
                .lbase (tuple): (x, y) coordinates of left base point
                .rbase (tuple): (x, y) coordinates of right base point
    ...
    def __get_keypoint_idxs(self, contour, peakpoint, basepoints):
        ...
        left_pt_idx = np.where((contour == basepoints["lbase"]).all(axis=-1))[0][0]
        right_pt_idx = np.where((contour == basepoints["rbase"]).all(axis=-1))[0][0]
